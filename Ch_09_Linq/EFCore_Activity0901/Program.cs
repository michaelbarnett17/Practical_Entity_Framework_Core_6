...
class Program
{
    ...
    {
        return $"ID: {sp.BusinessEntityId} \t"
            + $"TID: {sp.TerritoryId} \t" 
            + $"Quota: {sp.SalesQuota} \t"
            + $"Bonus: {sp.Bonus} \t" 
            + $"YTDSales: {sp.SalesYtd} \t" 
            + $"Name: {sp.BusinessEntity?.BusinessEntity?.FirstName ?? ""} {sp.BusinessEntity?.BusinessEntity?.LastName ?? ""}";
    }
    ...
    private static void ListAllSalespeople()
    {
        ...
        //   -----------------      -----------------      -----------------
        //  |                 |    |                 |    |                 |
        //  |   SalesPerson   |----|   Employee      |----|     Person      | 
        //  |                 |    |                 |    |                 |
        //   -----------------      -----------------      -----------------

        // SalesPerson.cs has: public virtual Employee BusinessEntity
        // Employee.cs has: public virtual Person Business Entity
    ...
    // Faster with Projections!
    private static void ShowAllSalespeopleUsingProjection()
    {
        using (var db = new AdventureWorksContext(_optionsBuilder.Options))
        {
            var salespeople = db.SalesPeople
                                // NEXT 2 LINES ARE NOT NEEDED WITH PROJECTIONS
                                //.Include(x => x.BusinessEntity)
                                //.ThenInclude(y => y.BusinessEntity)
                                .AsNoTracking()
                                .Select(x => new
            ...
            {
                Console.WriteLine(
                    $"BID: {sp.BusinessEntityId} \t"
                    + $"Name: {sp.LastName}, {sp.FirstName} \t"
                    + $"Quota: {sp.SalesQuota} \t"
                    + $"YTD Sales: {sp.SalesYtd} \t"
                    + $"Sales Last Year: {sp.SalesLastYear}");
            }
        }
    }
